...
#pragma pack(pop)
	...
	void send() {
		...
		send_stream.write((const char*) &header, sizeof(dns_header_t));

		for (std::string service : search) {
			uint8_t service_str_len = service.size();
			send_stream.write((const char*) &service_str_len, 1);
			send_stream.write(service.data(), service.size());
		}

		auto query = dns_query_t();
		query.type = htons(0x0C); // PTR
		query.class_ = htons(0x1); // IN
		send_stream.write((const char*) &query, sizeof(dns_query_t));

		socket.async_send_to(output_buffer.data(), multicast, [this](asio::error_code ec, size_t bytes) {
		});
	}
...
public:
	...
	void run_for(float duration) {
		...
		io_context.run_for(std::chrono::milliseconds((int)(duration * 1000)));
		socket.close();
		io_context.run();
	}
};
